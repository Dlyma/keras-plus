...
def load_data(sz, maxlen, train_split, cluster_num, norm, step):
    ...
    (X, Y, mins, maxs) = stock_cluster.load_data(csv_path='/home/zhaowuxia/dl_tools/datasets/stock/clusters/cluster%d.csv'%(cluster_num), norm = norm, sz = sz, maxlen = None, step = step, reverse=False ) 
...
if __name__=='__main__':
    ...
    model = build_model()
    #model.load_weights('models/cluster/model_%s_%d_%d'%(cluster_num, nb_sample, train_days))
    model.fit(X_train, y_train, batch_size=batch_size, nb_epoch=nb_epoch, verbose=1, validation_data=(X_test,  y_test), save_path='models/cluster/model_%s_%d_%d'%(cluster_num, nb_sample, train_days))
    model.save_weights('models/cluster/1lstm256/model_%s_%d_%d_final'%(cluster_num, nb_sample, train_days), overwrite=True)
    model.load_weights('models/cluster/model_%s_%d_%d'%(cluster_num, nb_sample, train_days))
    ...
    for i in range(10):
        write_csv('csv/cluster/1lstm256_output_%s_%d/%d_%d_num%d.csv'%(cluster_num, nb_sample, train_days, 2*test_days, i), gnd[i], pred1[i])
        write_csv('csv/cluster/1lstm256_output_%s_%d/%d_%d_num%d.csv'%(cluster_num, nb_sample, train_days/2, train_days/2+2*test_days, i), gnd[i], pred2[i])
